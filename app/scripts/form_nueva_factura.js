    ...
    .then((data) => {
      if (data) {
        ...
        Object.entries(data).forEach(([id, sucursal]) => {
          ...
          option.setAttribute("data-ptoEmi", sucursal.pto_emision); // Guardar el punto de emisión
          sucursalSelect.appendChild(option);
        });
        // Seleccionar la primera sucursal por defecto
        sucursalSelect.selectedIndex = 0;
        // Mostrar el punto de emisión de la sucursal seleccionada
        document.getElementById("numero").value = sucursalSelect.options[sucursalSelect.selectedIndex].getAttribute("data-ptoEmi");
      } else {
    ...
    .then((data) => {
      if (data) {
        ...
        Object.entries(data).forEach(([_, producto]) => {
          ...
          const requiereMes =
            producto.codigo === "EXC001" || producto.codigo === "TB0001";
